...
int perturb_initial_conditions(struct precision * ppr,
  ...
  if (_scalars_) {
    ...
    /*New EDE*/
    /*NEDE is irrelevant at the time where perturbations are initialised.*/
    ...
    */
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      /*New EDE: Convert inital conditions to Newtonian gauge / danger: we never have tested this part as we only use synchronous gauge*/
      if (pba->has_NEDE_pert == _TRUE_) {
...
int perturb_total_stress_energy(
  ...
  if (_scalars_) {
	...
	if ( 1. / a - 1.  > pba->z_decay) {
	  ...
	  rho_plus_p_tot += (1. + pba->three_eos_NEDE/3.) * ppw->pvecback[pba->index_bg_rho_NEDE];
	}
      }
  ...
  rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_trigger]+ppw->pvecback[pba->index_bg_p_trigger];
      }
    }
